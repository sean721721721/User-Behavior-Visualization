  ...
  function adjacencyMatrixNoAuthor(similarity, simThresh, artThresh) {
    ...
    function computeArticleSimilarity(articleArray, userArray) {
      ...
      for (let i = 0; i < articleArray.length; i += 1) {
        ...
        for (let j = i + 1; j < articleArray.length; j += 1) {
          ...
          const sim = intersectUsers / (temp.messages.length + next.messages.length - intersectUsers);
          if (sim) {
            array.push({
    ...
    function drawUserGroupBipartiteRelations() {
      ...
      function drawRelationRatio(index, userCount) {
        ...
        const groupArtGroupHp = articleGroupHeatmap.select(`.group_${index}`);
        groupArtGroupHp.selectAll('path')
          .data(communityEachLevelCount)
          .enter()
          .append('rect')
          .attr('y', (d, _index) => {
            let pos = 0;
