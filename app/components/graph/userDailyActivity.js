...
export default function userDailyActivity(data, user, svg, begin, end) {
  ...
  function update(date1, date2) {
    ...
    const myColor = d3.scaleLinear()
      ...
      .domain([0, 10]);
    const xScale = getXScale(date1, date2);
    const timeScale = d3.scaleTime().domain([new Date(date1), new Date(date2)]).range([0, 500]);
    const userScale = d3.scaleBand().domain(user).range([0, 100]);
    const yDomain = getYDomain(date1, date2);
    // console.log(yDomain);
    ...
    for (let i = 0; i < data.length; i += 1) {
      ...
      svg.append('g')
        ...
        .attr('x', (d, index) => {
          ...
          console.log(new Date(commentTime));
